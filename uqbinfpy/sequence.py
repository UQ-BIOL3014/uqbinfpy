    ...
    def writeHTML(self, filename):
        ...
        for seq in self.seqs:
            ...
            for sym in seq:
                ...
                html += '<font bgcolor="%s">%s</font>' % (color, sym) # shorter than above
            html += '\n'
            fh.write(html)
...
def getSequence(id, database = 'uniprotkb', start=None, end=None):
    ...
    except (RuntimeError, IndexError) as e1:
        raise RuntimeError('An error occurred while retrieving the specified sequence: %s; %s' % (id, e1))
